          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(340px,.82fr)_minmax(0,1.35fr)_300px]">
            ...
            <section className="overflow-hidden rounded-2xl border border-[#d8e0e3] bg-white shadow-[0_10px_32px_rgba(21,42,54,.06)]">
              ...
              <div className="p-5">
                ...
                <div className="mt-5 rounded-2xl border border-[#dce5e8] bg-[#f8fafb] p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="font-extrabold">这份通告对你有帮助吗？</p><p className="mt-1 text-sm text-slate-500">每个登录账号只能评价一次</p></div><div className="flex gap-2"><Button variant={feedback.myVote === "helpful" ? "default" : "outline"} disabled={!!feedback.myVote || feedbackLoading} onClick={() => vote("helpful")}><ThumbsUp />有帮助 {feedback.helpful}</Button><Button variant={feedback.myVote === "unhelpful" ? "default" : "outline"} disabled={!!feedback.myVote || feedbackLoading} onClick={() => vote("unhelpful")}><ThumbsDown />需改进 {feedback.unhelpful}</Button></div></div>{feedbackMessage && <p className="mt-3 text-sm font-semibold text-[#176a62]">{feedbackMessage}</p>}</div>
              </div>
            </section>
